...
class MultiImageMixDataset:
    ...
    def __getitem__(self, idx):
        ...
        for (transform, transform_type) in zip(self.pipeline,
            ...
            #     show_img = results["img"].astype(np.uint8).copy()
            #     for idx, box in enumerate(results["gt_bboxes"]):
            #         cv2.rectangle(show_img, (int(box[0]), int(box[1])), (int(box[2]), int(box[3])), (0, 0, 255), 1)
            #         cv2.putText(show_img, self.CLASSES[results["gt_labels"][idx]], (int(box[0]), int(box[1])),
            #                     cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 1)
            #     os.makedirs("data/xray/vis_trans", exist_ok=True)
            #     cv2.imwrite("data/xray/vis_trans/%s_%s.jpg" % (idx, transform_type),
            #                 show_img)
