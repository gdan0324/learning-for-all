...
@PIPELINES.register_module()
class AutoAugmentWithTemplate(object):
    def __init__(self, autoaug_type="v1"):
        """
        Args:
            autoaug_type (str): autoaug type, support v0, v1, v2, v3, test
        """
        super(AutoAugmentWithTemplate, self).__init__()
        self.autoaug_type = autoaug_type
    ...
    def __call__(self, results):
        ...
        from mmdet.datasets.pipelines.autoaugment_utils import distort_image_with_autoaugment_with_template
        im, template, norm_gt_bbox = distort_image_with_autoaugment_with_template(im, template, norm_gt_bbox,
                                                                                  self.autoaug_type)
        gt_bbox[:, 0] = norm_gt_bbox[:, 1] * float(width)  # x1
        gt_bbox[:, 1] = norm_gt_bbox[:, 0] * float(height)  # y1
        gt_bbox[:, 2] = norm_gt_bbox[:, 3] * float(width)  # x2
