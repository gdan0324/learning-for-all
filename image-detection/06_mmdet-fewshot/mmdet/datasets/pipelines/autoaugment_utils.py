...
def _apply_multi_bbox_augmentation(image, bboxes, prob, aug_func,
    ...
    wrapped_aug_func = lambda _image, bbox, _new_bboxes: _apply_bbox_augmentation_wrapper(_image, bbox, _new_bboxes,
                                                                                          prob, aug_func,
                                                                                          func_changes_bbox, *args)
...
def select_and_apply_random_policy_with_template(policies, image, template, bboxes):
    """Select a random policy from `policies` and apply it to `image`."""
    policy_to_select = np.random.randint(0, len(policies), dtype=np.int32)
    # policy_to_select = 6 # for test
    for (i, policy) in enumerate(policies):
        if i == policy_to_select:
            image, bboxes = policy(image, bboxes)
            template, _ = policy(template, np.copy(bboxes))
    return image, template, bboxes
...
def build_and_apply_nas_policy_with_template(policies, image, template, bboxes, augmentation_hparams):
    """Build a policy from the given policies passed in and apply to image.
    Args:
        template:
        policies: list of lists of tuples in the form `(func, prob, level)`, `func`
    ...
    augmented_images, augmented_template, augmented_bboxes = select_and_apply_random_policy_with_template(
        tf_policies, image, template, bboxes)
    # If no bounding boxes were specified, then just return the images.
    return augmented_images, augmented_template, augmented_bboxes
...
def distort_image_with_autoaugment_with_template(image, template, bboxes, augmentation_name):
    """Applies the AutoAugment policy to `image` and `bboxes`.
    Args:
        template:
        image: `Tensor` of shape [height, width, 3] representing an image.
    ...
    policy = available_policies[augmentation_name]()
    augmentation_hparams = {}
    return build_and_apply_nas_policy_with_template(policy, image, template, bboxes,
                                                    augmentation_hparams)
