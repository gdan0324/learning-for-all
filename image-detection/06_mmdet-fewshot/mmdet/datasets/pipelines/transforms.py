...
class RandomCenterCropPad:
    ...
    def __call__(self, results):
        # 以下是自己加的
        if results['img'].dtype != np.float32:
        ...
        if self.test_mode:
            ...
            # 因为pipeline一般为uint8，所以还是转成uint8，有问题再改
...
class MixUp:
    ...
    def _mixup_transform(self, results):
        ...
        results['img'] = mixup_img.astype(np.uint8)  # np.uint8
...
class CopyPaste:
    ...
    def _copypaste_transform(self, results):
        ...
        # 2. scale jit
        scale_ratio *= jit_factor
        out_img = mmcv.imresize(retrieve_img, (int(retrieve_img.shape[1] * jit_factor),
                                               int(retrieve_img.shape[0] * jit_factor)))

        # 5. random select object
        ori_img = results['img']
        out_h, out_w = out_img.shape[:2]
        target_h, target_w = ori_img.shape[:2]

        gt_idx = random.randint(0, len(retrieve_results['gt_bboxes']))
        retrieve_gt_bbox = retrieve_results['gt_bboxes'][gt_idx]

        # 5. adjust bbox
        retrieve_gt_bbox[0::2] = retrieve_gt_bbox[0::2] * scale_ratio
        retrieve_gt_bbox[1::2] = retrieve_gt_bbox[1::2] * scale_ratio
        bbox_w = retrieve_gt_bbox[2] - retrieve_gt_bbox[0]
        bbox_h = retrieve_gt_bbox[3] - retrieve_gt_bbox[1]

        # 8. copy paste 往外扩展10像素点
        ori_img = ori_img.astype(np.float32)
        ...
        right_border = 10
        bottom_border = 10
        if retrieve_gt_bbox[0] < 10:
            left_border = retrieve_gt_bbox[0]
        if retrieve_gt_bbox[1] < 10:
            top_border = retrieve_gt_bbox[1]
        if out_h - retrieve_gt_bbox[3] < 10:
            bottom_border = out_h - retrieve_gt_bbox[3]
        if out_w - retrieve_gt_bbox[2] < 10:
            right_border = out_w - retrieve_gt_bbox[2]

        obj = out_img[
              int(retrieve_gt_bbox[1] - top_border):int(retrieve_gt_bbox[3] + bottom_border),
              int(retrieve_gt_bbox[0] - left_border):int(retrieve_gt_bbox[2] + right_border), :
              ]
        if target_w < obj.shape[1] * 2 or target_h < obj.shape[0] * 2:
            return results

        if is_flip:
            obj = obj[:, ::-1, :]
        offset_x = random.randint(0, target_w - obj.shape[1])
        offset_y = random.randint(0, target_h - obj.shape[0])
        # 往外扩10像素点的宽高
        with_bg_h = obj.shape[0]
        with_bg_w = obj.shape[1]
        ori_img[offset_y:offset_y + with_bg_h, offset_x:offset_x + with_bg_w, :] = obj

        retrieve_gt_label = retrieve_results['gt_labels'][gt_idx]
        if not self.skip_filter:
            raise ValueError("暂时只支持skip_filter=True")
        ...
        mixup_gt_bboxes = np.concatenate([
            ...
            [[
                ...
                offset_y + top_border,
                offset_x + with_bg_w - right_border,
                offset_y + with_bg_h - bottom_border
            ]]
        ], axis=0)
        mixup_gt_labels = np.concatenate([results['gt_labels'], [retrieve_gt_label]], axis=0)
        ...
        results['img'] = ori_img.astype(np.uint8)  # np.uint8
...
@PIPELINES.register_module()
class RandomCenterCropPadWithTemplate:
    """Random center crop and random around padding for CornerNet.
    ...
    def _train_aug(self, results):
        ...
        while True:
            ...
            for i in range(50):
                ...
                cropped_img, border, patch = self._crop_image_and_paste(
                    img, [center_y, center_x], [new_h, new_w])
                cropped_template_img, template_border, template_patch = self._crop_image_and_paste(
                    template_img, [center_y, center_x], [new_h, new_w])
                mask = self._filter_boxes(patch, boxes)
                # if image do not have valid bbox, any crop patch is valid.
                if not mask.any() and len(boxes) > 0:
                    continue
                ...
                results['img_shape'] = cropped_img.shape
                results['pad_shape'] = cropped_img.shape
                results['template_img'] = cropped_template_img
    ...
    def __call__(self, results):
        ...
        if results['img'].dtype != np.float32:
            results['img'] = results['img'].astype(np.float32)
        if results['template_img'].dtype != np.float32:
            results['template_img'] = results['template_img'].astype(np.float32)
        # -----------------------------------------------------
        img = results['img']
        assert img.dtype == np.float32, (
            'RandomCenterCropPad needs the input image of dtype np.float32,'
        ...
        if self.test_mode:
            ...
            results = self._train_aug(results)
            results['img'] = results['img'].astype(np.uint8)
            results['template_img'] = results['template_img'].astype(np.uint8)
            return results
...
@PIPELINES.register_module()
class MixUpWithTemplate:
    def __init__(self, p=0.5, max_iters=10):
        self.p = p  # mixup的概率
        self.max_iters = max_iters
    ...
    def _mixup_transform(self, results):
        ...
        retrieve_results = results['mix_results'][0]
        retrieve_img = retrieve_results['img']
        out_img = np.zeros_like(results['img'])
        # 1. keep_ratio resize
        scale_ratio = min(results['img_shape'][0] / retrieve_img.shape[0],
                          results['img_shape'][1] / retrieve_img.shape[1])
        retrieve_img = mmcv.imresize(
            retrieve_img, (int(retrieve_img.shape[1] * scale_ratio),
                           int(retrieve_img.shape[0] * scale_ratio)))
        ...
        mixup_img = 0.5 * ori_img + 0.5 * out_img.astype(np.float32)
