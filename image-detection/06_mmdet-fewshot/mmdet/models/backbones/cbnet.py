...
class _ConvNeXt(ConvNeXt):
    # 删除第二个backbone的第一个downsample层
    def del_layers(self):
        del self.downsample_layers[0]
    ...
    def forward(self, x, cb_feats=None, pre_tmps=None):
        ...
        tmps = []
        # 第一个backbone会进行stride=4的下采样
        # 第二个backbone则直接把backbone1的结果拿过来
        if len(self.downsample_layers) == 4:
        ...
        for i in range(len(self.stages)):
            ...
            tmps.append((x_out, H, W))
            if i < len(self.stages) - 1:
                x = self.downsample_layers[downsample_idx + i](x_out)
    ...
    def train(self, mode=True):
        """Convert the model into training mode while keep layers freezed."""
        super(_ConvNeXt, self).train(mode)


@BACKBONES.register_module()
class CBConvNeXt(BaseModule):
    def __init__(self, in_chans, depths, dims,
                 drop_path_rate, layer_scale_init_value, out_indices, cb_zero_init=True, **kwargs):
        super(CBConvNeXt, self).__init__()
        self.cb_zero_init = cb_zero_init
        self.cb_modules = nn.ModuleList()
        for cb_idx in range(2):
            cb_module = _ConvNeXt(in_chans=in_chans, depths=depths, dims=dims,
                                  drop_path_rate=drop_path_rate, layer_scale_init_value=layer_scale_init_value,
                                  out_indices=out_indices, **kwargs)
            if cb_idx > 0:
                cb_module.del_layers()
            self.cb_modules.append(cb_module)
        ...
        for i in range(self.num_layers):
            ...
            for j in range(jrange):
                if dims[i + j] != dims[i]:
                    layer = nn.Conv2d(dims[i + j], dims[i], 1)
                else:
                    layer = nn.Identity()
                linears.append(layer)
            self.cb_linears.append(linears)
    ...
    def _get_cb_feats(self, feats, tmps):
        ...
        for i in range(self.num_layers):
            ...
            for j in range(jrange):
                tmp = self.cb_linears[i][j](feats[j + i])
                tmp = F.interpolate(tmp, size=(Wh, Ww), mode='nearest')
                feed += tmp
            cb_feats.append(feed)
    ...
    def train(self, mode=True):
        """Convert the model into training mode while keep layers freezed."""
        super(CBConvNeXt, self).train(mode)
        for m in self.cb_modules:
            m.train(mode=mode)
        for m in self.cb_linears.modules():
            # trick: eval have effect on BatchNorm only
